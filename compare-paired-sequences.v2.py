...
CompareResult = namedtuple('CompareResult',
                           'comparison_name, anchor_name, ref_name, alphabet, ksize, scaled, jaccard, max_containment, anchor_containment, anchor_hashes, query_hashes, num_common')
...
def compare_sigs(sigA, sigB, comparison_name, lowest_common_rank, path_name, alpha, ksize, scaled):
    ...
    return CompareResult(comparison_name, str(sigA).split(" ")[0], str(sigB).split(" ")[0], alpha, ksize, scaled, jaccard, max_contain, containA, sigA_numhashes, sigB_numhashes, intersect_numhashes)
...
def main(args):
    ...
    compare_info = pd.read_csv(args.siminfo_csv, dtype=str, sep="\t", header=0)
    ...
    seq_comparisons = []
    for n, comparison_name in enumerate(names):
        if n !=0 and n % 50 == 0:
        ...
        for sc in scaled:
            ...
            comparison = compare_sigs(anchor_sig, compare_sig, comparison_name, lowest_common_taxon, path, alphabet, ksize, scaled)
